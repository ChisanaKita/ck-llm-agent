...
from ..models.api import ChatCompletionRequest, ChatCompletionResponse, ChatMessage, Choice, TokenUsage
from ..models.internal import AgentTask, AgentConfig, TaskStatus
...
class TaskProcessor:
    ...
    def __init__(
        ...
        logger.info(f"TaskProcessor initialized: caching={enable_caching}, cache_ttl={cache_ttl}s")
    ...
    async def process_chat_request(
        ...
        try:
            logger.info(f"Processing chat request: model={request.model}, messages={len(request.messages)}")
            ...
            all_messages = self._merge_context_messages(context_messages, request.messages)
            ...
            self._update_conversation_context(conversation_id, request.messages, task.result)
    ...
    def _merge_context_messages(
        ...
        while estimated_tokens > self.max_context_length and len(all_messages) > len(new_messages):
    ...
    async def cleanup(self) -> None:
        ...
        old_conversations = [
            conv_id for conv_id, last_access in self._conversation_access.items()
        ...
        logger.info(f"Cleanup completed: removed {len(expired_keys)} cache entries, {len(old_conversations)} old conversations")
